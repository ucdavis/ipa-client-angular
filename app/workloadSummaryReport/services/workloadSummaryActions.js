		...
		return {
			...
			_calculateView: function () {
				...
				instructors.ids.forEach(function(instructorId) {
					...
					instructorAssignments.forEach(function(teachingAssignment) {
						...
						if (teachingAssignment.sectionGroupId > 0) {
							...
							assignment.units = CourseService.getUnits(sectionGroup);
							assignment.studentCreditHours = CourseService.getSCH(assignment.seats, sectionGroup);
				...
				var unassignedCourses = unassignedSectionGroupIds.map(function (sectionGroupId) {
					...
					unassignedCourse.units = CourseService.getUnits(sectionGroup);
					unassignedCourse.studentCreditHours = CourseService.getSCH(unassignedCourse.seats, sectionGroup);
