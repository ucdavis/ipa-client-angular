class WorkloadSummaryService {
	constructor(ApiService, $q, $http, $window) {
		...
		return {
			...
			downloadHistorical: function (workgroupId, year) {
				var deferred = $q.defer();
	
				$http.get(window.serverRoot + "/api/workloadSummaryReport/" + workgroupId + "/years/" + year + "/generateHistoricalExcel", { withCredentials: true })
					.then(function (payload) {
						$window.location.href = payload.data.redirect;
						deferred.resolve(payload.data);
					},
