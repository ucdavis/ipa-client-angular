	...
	constructor () {
		return {
			generatePreferences: function (preferences, courses, sectionGroups) {
				...
				return addDisplayRanks(newPreferences);
			},
			generatePotentialPreferences: function (supportAssignments, courses, sectionGroups, termCode) {
				var self = this;
				...
				let currentTermSectionGroups = Object.values(sectionGroups.list).filter(sg => sg.termCode === termCode);

				currentTermSectionGroups.forEach(function(sectionGroup) {
